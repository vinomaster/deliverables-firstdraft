...
def handle_web_resource(uri, rfcs, cache):
    ...
    if uri in cache:
        ...
        if not error:
            ...
            if r.statuscode < 200 or r.statuscode > 299:
                error = "returns HTTP status code " + str(r.statuscode)
            else:
...
def main(full_check = False):
    error_count = 0
    folders = [x for x in map(lambda x: os.path.join(ROOT_FOLDER, x), ["specs","templates","definitions","recommendations","plans","white_papers"]) if os.path.isdir(x)]
    rfcs = []
